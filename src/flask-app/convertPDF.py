    ...
    if not data:
        ...
        for item in data:
            if (pdf.page_no() == 1 and row_count >= first_page_rows) or (pdf.page_no() > 1 and row_count >= max_rows_per_page):
                ...
                row_count, image_count = 0, 0
            base64_list = item["imageTitle"].split(',')
            imgdata = base64.b64decode(base64_list[0])
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmpfile:
                tmpfile.write(imgdata)
                tmp_filename = tmpfile.name
            pdf.image(tmp_filename, x=current_x + 20, y=current_y, w=image_width, h=image_height)
